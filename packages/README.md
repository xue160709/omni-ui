...
- `@multimodal-ui/shadcn`：可选的 shadcn registry wrapper，安装到 `components/multimodal/*`，不覆盖 `components/ui/*`。
...
如果你使用 shadcn registry wrapper，按项目里的 registry 地址安装对应组件；wrapper 只是帮你少写 `useInteractionNode`/`MultimodalGroup`，底层 runtime 仍来自 `@multimodal-ui/react`。
...
## 9. shadcn Wrapper Usage

如果你装了 shadcn wrapper，可以用 `components/multimodal/*` 里的组件替换部分基础 UI 组件。
...
wrapper 适合基础控件。业务对象仍建议用 `MultimodalGroup` 表达边界和 `entity`。
