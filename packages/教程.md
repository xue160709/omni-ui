...
| `confirmation_required` | 高风险操作需要用户确认 | 保存返回的 `pendingCommand.turnId`，调用 `confirmTurn(turnId)` 确认同一条冻结命令 |
...
- 操作输出格式说明：优先返回 `interaction_hypotheses`，只提供 intent、targetReference、slots 和 confidence；旧式 `interaction_action` JSON 仍作为 command proposal 兼容。
...
7. 用 `useInteractionApi()`、`useVoiceAdapter()` 或 `useAssistantConversation()` 接入输入框、ASR 语音或聊天。
