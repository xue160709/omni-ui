...
这份教程面向**应用开发者**，说明如何把 Multimodal UI 接入已有的 React 项目。
...
不需要手动安装 `@multimodal-ui/core`，也不需要安装 `@multimodal-ui/shadcn`。

详细说明：

- `@multimodal-ui/react` 已经依赖 `@multimodal-ui/core`，并且会重新导出常用的 core API。
- `@multimodal-ui/shadcn` 只是可选的 shadcn registry / starter recipe，不是 runtime 必需品。
...
| 包 | 什么时候用 | 是否需要手动安装 |
| --- | --- | --- |
| `@multimodal-ui/react` | React 应用接入多模态能力。Provider、页面/对象标记、route/action 注册、命令输入、assistant 都在这里。 | 大多数项目只装这个 |
| `@multimodal-ui/core` | 非 React adapter、服务端 resolver、测试环境、框架无关类型和校验能力。 | React 项目通常不用手动装 |
| `@multimodal-ui/shadcn` | 你想用 shadcn CLI 安装可编辑的 `components/multimodal/*` wrapper 或 starter recipe。 | 可选，需要 shadcn 风格源码时再用 |

换句话说：

- 已有 React 项目：装 `@multimodal-ui/react`。
- 新 React 项目，但还没决定 UI 库：还是先装 `@multimodal-ui/react`。
- 已经在用 shadcn/ui，并且想要现成的 Button、Input、Dialog、DataTable、AssistantPanel wrapper：再加 shadcn registry item。
- 要给 Vue、Svelte、Web Components 或服务端流程做底层集成：才直接用 `@multimodal-ui/core`。

## 快速问答

**`@multimodal-ui/react` 需要安装 `@multimodal-ui/core` 吗？**

不需要。`@multimodal-ui/react` 的 `dependencies` 里已经包含 `@multimodal-ui/core`。在 React 应用里也优先从 `@multimodal-ui/react` import，它会重新导出常用类型和工具。

**`@multimodal-ui/react` 需要安装 `@multimodal-ui/shadcn` 吗？**

不需要。shadcn 是可选源码配方。只有当你想用 shadcn CLI 生成 `components/multimodal/*` 这些可编辑 wrapper 时，才使用 registry item。

**三个包需要都安装吗？**

通常不需要。React 应用默认只安装 `@multimodal-ui/react`。你明确要写非 React adapter 或服务端 resolver 时再直接安装 `@multimodal-ui/core`；你明确要 shadcn wrapper 时再使用 shadcn registry。
...
然后从 `@multimodal-ui/react` 引入所有 React runtime API：
...
registry item 会把可编辑源码安装到你的 `components/multimodal/*`。这些文件底层仍然 import `@multimodal-ui/react`，不会替换或覆盖你的 `components/ui/*`。
...
```tsx
import * as React from "react"
import { useAssistantConversation } from "@multimodal-ui/react"
