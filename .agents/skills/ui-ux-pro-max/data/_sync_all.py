#!/usr/bin/env python3
"""
根据最新的 products.csv（约 161 行）同步 colors.csv 与 ui-reasoning.csv。
- 删除已移除的产品类型
- 按映射表重命名不一致的条目
- 为缺失的产品类型补全新行
- 保持 colors.csv 与 products.csv 一一对应
- 重新编号
"""
import csv, os, json
...
def derive_row(pt, pri, sec, acc, bg, notes=""):
    """由 4 个基础色生成完整一行（含 16 个语义色 token 列）。"""
    dark = is_dark(bg)
...
# ─── 新增配色定义：(主色, 次色, 强调色, 背景, 备注) ──────────
# 按类别分组便于维护；每个元组经 derive_row 展开为一整行。
NEW_COLORS = {
    # ── 原 #97–#116 段曾缺省配色，在此补全 ──
...
# ─── 2. 重建 ui-reasoning.csv ─────────────────────────────────────────────
def derive_ui_reasoning(prod):
    """由 products.csv 的一行推导 ui-reasoning.csv 对应行（风格/落地页等字段）。"""
    pt = prod["Product Type"]
